    ...
    if (usersData?.length > 0) {
      ...
      usersData.map((user) => {
        if (user?.geo_data?.latitude) {
          latLongData.push({ lat: user?.geo_data?.latitude, lng: user?.geo_data?.longitude, size: 0.5, user: user?.username })
        }
      })
      console.log('latLongDataL ', latLongData);
      setPointsData(latLongData);
      fetch('https://globe.gl/example/datasets/ne_110m_populated_places_simple.geojson').then(res => res.json()).then(places => setPlaces(places))
    }
  }, [usersData])
